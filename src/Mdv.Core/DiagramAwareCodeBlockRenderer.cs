...
public sealed class DiagramAwareCodeBlockRenderer(CodeBlockRenderer? fallback) : HtmlObjectRenderer<CodeBlock>
{
    ...
    private static void WriteDiagram(HtmlRenderer renderer, DiagramKind kind, DiagramRequest request, string source)
    {
        ...
        renderer.Write("<figure class=\"mdv-diagram mdv-diagram-")
            .Write(kindClass)
            .Write("\" data-mdv-diagram=\"")
            .Write(kindClass)
            .Write("\" data-mdv-key=\"")
            .Write(request.Key)
            .Write("\">");
        ...
        else
        {
            ...
            renderer.Write("<div class=\"mdv-d2-slot\" data-mdv-key=\"")
                .Write(request.Key)
                .Write("\"><div class=\"mdv-diagram-placeholder\">Rendering D2 diagram…</div></div>");
        }
