// mdv browser front-end: live reload, diagrams, search, TOC, multi-file nav.
...
const statusEl = document.getElementById("mdv-status");
const statusText = document.getElementById("mdv-status-text");
...
async function renderMermaid(root) {
  ...
  const blocks = root.querySelectorAll("figure.mdv-diagram-mermaid pre.mermaid:not([data-mdv-done])");
  for (const pre of blocks) {
    ...
    try {
      ...
      fig.innerHTML = svg;
      fig.setAttribute("data-mdv-done", "1");
    } catch (e) {
      pre.setAttribute("data-mdv-done", "1");
      pre.outerHTML = `<div class="mdv-diagram-error">Mermaid error: ${escapeHtml(String(e))}</div>`;
    }
  }
}

async function renderD2(root) {
  const slots = root.querySelectorAll(".mdv-d2-slot:not([data-mdv-done])");
  for (const slot of slots) {
    const key = slot.getAttribute("data-mdv-key");
    slot.setAttribute("data-mdv-done", "1");
    try {
      const resp = await fetch(`/_mdv/diagram/${key}?theme=${currentTheme}&format=svg`);
      if (!resp.ok) throw new Error(await resp.text());
      slot.innerHTML = await resp.text();
    } catch (e) {
      slot.innerHTML = `<div class="mdv-diagram-error">D2 error: ${escapeHtml(String(e))}</div>`;
    }
  }
}
...
function highlight(root) {
  ...
  root.querySelectorAll("pre code").forEach((block) => {
    if (block.closest(".mdv-diagram")) return;
...
function buildToc() {
  ...
  content.querySelectorAll("[data-mdv-toc-inline]").forEach((nav) => {
    nav.innerHTML = "";
    entries.forEach(({ h }) => {
      const level = Number(h.tagName.substring(1));
      const a = document.createElement("a");
...
// ---------------- search ----------------
const searchInput = document.getElementById("mdv-search");
const searchCount = document.getElementById("mdv-search-count");
let hits = [];
let hitIndex = -1;

function clearSearch() {
  content.querySelectorAll("mark.mdv-hit").forEach((m) => {
...
document.getElementById("mdv-search-next").addEventListener("click", () => nextHit(1));
document.getElementById("mdv-search-prev").addEventListener("click", () => nextHit(-1));
...
async function navigate(path, push = true) {
  ...
  try {
    ...
    document.getElementById("mdv-doc-title").textContent = data.title;
...
function interceptLinks() {
  content.addEventListener("click", (e) => {
    ...
    const local = a.getAttribute("data-mdv-local");
    if (local) {
      e.preventDefault();
      navigate(local);
    }
...
// ---------------- theme ----------------
document.getElementById("mdv-theme-toggle").addEventListener("click", async () => {
  ...
  content.querySelectorAll("figure.mdv-diagram-mermaid[data-mdv-done]").forEach((f) => f.removeAttribute("data-mdv-done"));
  content.querySelectorAll(".mdv-d2-slot[data-mdv-done]").forEach((s) => { s.removeAttribute("data-mdv-done"); s.innerHTML = '<div class="mdv-diagram-placeholder">Rendering D2 diagram…</div>'; });
  await renderAll(content);
});

document.getElementById("mdv-back").addEventListener("click", () => history.back());
document.getElementById("mdv-forward").addEventListener("click", () => history.forward());

// ---------------- view toggles (sidebar / toolbar / zen) ----------------
const layout = document.getElementById("mdv-layout");
const toolbar = document.getElementById("mdv-toolbar");
function toggleSidebar() {
  document.body.classList.toggle("mdv-no-sidebar");
  flashStatus(document.body.classList.contains("mdv-no-sidebar") ? "Sidebar hidden" : "Sidebar shown");
}
function toggleToolbar() {
  document.body.classList.toggle("mdv-no-toolbar");
  flashStatus(document.body.classList.contains("mdv-no-toolbar") ? "Toolbar hidden" : "Toolbar shown");
}
function toggleZen() {
  const on = !document.body.classList.contains("mdv-zen");
  document.body.classList.toggle("mdv-zen", on);
  document.body.classList.toggle("mdv-no-sidebar", on);
  document.body.classList.toggle("mdv-no-toolbar", on);
  flashStatus(on ? "Zen mode (z to exit)" : "Zen mode off");
}

// Hook up toolbar buttons that may exist in the shell.
document.getElementById("mdv-toggle-sidebar")?.addEventListener("click", toggleSidebar);
document.getElementById("mdv-toggle-zen")?.addEventListener("click", toggleZen);

// ---------------- help overlay (which-key style) ----------------
const helpOverlay = document.getElementById("mdv-help-overlay");
const HELP_SECTIONS = [
  { title: "Move", items: [
    ["j / k", "down / up a line"],
...
function buildHelp() {
  const body = document.getElementById("mdv-help-body");
  if (body.childElementCount) return; // build once
  for (const section of HELP_SECTIONS) {
    const group = document.createElement("div");
    group.className = "mdv-help-group";
...
document.addEventListener("keydown", (e) => {
  ...
  if (e.key === "Escape" && !helpOverlay.classList.contains("mdv-hidden")) { e.preventDefault(); toggleHelp(false); return; }
  ...
  switch (e.key) {
    ...
    case "[": e.preventDefault(); document.getElementById("mdv-theme-toggle").click(); break;
...
function showStatus(msg, isError) {
  ...
  statusEl.classList.remove("mdv-hidden");
  statusEl.style.borderColor = isError ? "#da3633" : "var(--border)";
}
function hideStatus() { statusEl.classList.add("mdv-hidden"); }
...
(async function init() {
  if (!currentPath) {
    currentPath = document.body.getAttribute("data-mdv-initial-path") || "";
  }
  interceptLinks();
  await afterContentChange(false);
  connectLiveReload();
